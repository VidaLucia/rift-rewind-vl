...
# -------------------------------------------------------
# Resolve paths dynamically regardless of where it's run
# -------------------------------------------------------
BASE_DIR = Path(__file__).resolve().parents[2]  # D:\Github\league-match-grabber
...
async def get_puuid(session, region: str, summoner_name: str, tag: str):
    ...
    print(f"[DEBUG] get_puuid: region={region}, routing={routing}")
    if not routing or routing == "none":
        raise ValueError(f"Invalid region '{region}' → got routing={routing}")
...
async def export_match_history(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, match_id in enumerate(match_ids[:max_matches] if max_matches else match_ids):
            ...
            try:
                ...
                for p in participants:
                    ...
                    row = {
                        ...
                        "date": match_data.get("info", {}).get("gameCreation", 0),
                        ...
                        "primarystyle_perk4": p.get("perks", {}).get("styles", [{}])[0].get("selections", [{}])[3].get("perk", 0),
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 4:
        print("Usage: python player_finder.py <region> <summoner_name> <tag> [max_matches]")
        sys.exit(1)

    region = sys.argv[1]
    summoner_name = sys.argv[2]
    tag = sys.argv[3]
    max_matches = int(sys.argv[4]) if len(sys.argv) > 4 else 200
    ...
    asyncio.run(
        export_match_history(
            ...
            summoner_name,
            tag,
            output_path = DATA_DIR / f"{summoner_name}#{tag}_data.csv",
